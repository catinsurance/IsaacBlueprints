...
Enemies make up an important part of Isaac's gameplay, providing challenging foes to face against in the many rooms that make up. This tutorial will cover the bare essentials for creating an enemy. This tutorial will *not* cover enemy AI or having them appear within rooms.
...
???- info "`entity` tag variables"
	| boss | int | Whether or not the entity is a boss. Possible values: [`0`, `1`]. Default = '0'. |
	...
	| champion | int | Allow champion variants of this entity. Possible values: [`0`, `1`]. Default = '0'. |
	| stageHP | int | A multiplier for how much additional health the entity gains with each stage. Read [here](https://bindingofisaacrebirth.wiki.gg/wiki/Stage_HP) for more information. |
	...
	| reroll | bool | Deprecated. Use the [noreroll tag](entity_basics.md#tags-explanation) instead. |
	...
	| bestiaryAnim | string | Unused. <br>See [Addiing the enemy to the Bestiary](enemies.md#adding-the-enemy-to-the-bestiary) for information on adding the enemy to the Bestiary. |
	| bestiaryOverlay | string | Unused. <br>See [Addiing the enemy to the Bestiary](enemies.md#adding-the-enemy-to-the-bestiary) for information on adding the enemy to the Bestiary. |
...
[Level 2 Roundy](enemies.md#entities-entry) has the following definition for their `devolve` tag:
```xml
<devolve id="276.0.0" weight="1" />
```
It only has one entry for devolving, set to Entity Type 276, Variant 0, Subtype 0, which is the entry for [Roundy](https://bindingofisaacrebirth.wiki.gg/wiki/Round_Worm#Roundy)
...
1. The `bestiary` child tag.
2. The `portrait` variable.
3. Encountering your enemy at least once within a run.
