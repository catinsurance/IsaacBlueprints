...
Characters are a core element and main attraction of the Binding of Isaac as a whole. You start only able to play as Isaac, but unlock up to 16 additional characters on top of all their tainted counterparts, totaling to 34. This tutorial will cover how to create your own character and how to play as them yourself

## Video tutorial
[![Characters | Youtube Tutorial](https://img.youtube.com/vi/y38xp-Hw1H0/0.jpg)](https://youtu.be/y38xp-Hw1H0 "Video tutorial")

## Getting the required files
A single character involves numerous files scattered across different folders in your mod. Download [this template](https://github.com/catinsurance/isaac-character-template) that will have all the required files, which will be referenced throughout this tutorial.

If you wish to reference and use existing files on each character in the vanilla game, they can be located in the game's directory inside `resources/gfx/characters/` (or `resources-dlc3` for Repentance content) after [extracting the game's resources](creating_a_mod.md/#extracting-the-games-resources).

## Creating your character
The very first step in creating a character is making an entry for your character in the players.xml file. This defines everything about the character such as their name, starting items, health, spritesheets, and much more!
...
???- info "player tag variables"
	...
	|pocketActive|int|Starting pocket active item ID|
	...
	|hidden|bool|True = the character won't appear in the selection screen|
...
???- info ":modding-repentogon: REPENTOGON-exclusive player tag variables"
	...
	|healthtype|int|The health type the player should start with. The int corresponds to the [HealthType](https://repentogon.com/xml/achievements.html) enum.|
	...
	|achievement|int or string|This is now respected on modded characters. For mods, it's best to define this as the *name* of the achievement (for example, `achievement='TaintedPeter'`, TaintedPeter being the name attribute of the achievement). Characters with this tag should have a second frame in their `characterportraits(alt).anm2` which should be the "locked" picture for the character. In addition, all stat/item layers on the main menu will be hidden and the `Unlocked By` layer will be visible. See the [achievements XML section](https://repentogon.com/xml/achievements.html) for more info.|
...
## Spritesheets
With the players.xml set up, you can now technically play as your new character, but they will have no visuals whatsoever, rendering them invisible apart from their shadow. The grand majority of work that goes into a character are their spritesheets as there are quite a number of them!
...
Tainted characters are "B-Side" versions of an existing character. Introduced in the Repentance DLC, they typically twist the mechanics of the character they're based on in a unique way. These are optional to create, and if left out, the game will display your normal character on both the normal and tainted character menus. If you do wish to create one, they require a specific setup so that they are registered as a tainted version of your normal character with their own set of sprites as opposed to just another character to select.
...
???+ note
	It is recommended to keep your tainted character's name the same as it's normal side. So, if your normal character's name is Gabriel, your tainted version's name should also be Gabriel.
![The full players.xml of your mod](../assets/characters/players_xml_tainted.png)

### Content anm2 files
Inside `content/gfx`, when making a tainted character, you will need to copy and rename the name of several anm2s so that they are distinctly for the tainted character instead of the normal character. Below is a list of all the available files you can rename and edit:
...
### Other
Of course, with a separate character of any kind, you will need to create all new spritesheets for everything your new character has. Be sure to update the content anm2s with your new sprites!

The only exceptions to these, which are optional and can be left the same if your character shares the same name, would be the vs. screen name and death name. All vanilla characters share the exact same sprites for these instances between their normal and tainted variants, but you can create a new sprite for them if you wish.