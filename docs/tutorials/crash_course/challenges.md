...
Challenges change the gameplay of a regular run for a unique twist. This tutorial will cover how to create your own and all the attributes you can modify unique to challenges.

## Creating your challenge

Creating a challenge requires the [challenges.xml](https://wofsauge.github.io/IsaacDocs/rep/xml/challenges.html) file. They require the root `challenges` tag and its child `challenge` tag, where you define your individual challenges. Below are explanations for their variables:
...
???- info "`challenge` tag variables"
	| Variable-Name | Possible Values | Description |
	|:--|:--|:--|
	|id|int|Ingame-Id of the Challenge (Used only by vanilla challenges. Use the `name` variable instead.)|
	|name|string|Name of the Challenge.|
	|startingitems|string|Comma separated (nospace) item ids that will be added at the start.|
	|startingitems2|string|Starting items for Player 2 (Esau, when playing as Jacob & Esau). Comma separated (nospace) item ids that will be added at the start.|
	|startingtrinkets|string|Comma separated (nospace) trinket ids that will be added at the start (max 2).|
	|startingcard|string| [Card id](../enums/Card.md) of the starting cards<br>Default is `-1` (no card).|
	|startingpill|string| [PillEffect id](../enums/PillEffect.md) of the starting pill<br>Default is `-1` (no pill).|
	...
	|roomfilter|string|Comma separated list of [RoomTypes](../enums/RoomType.md) to not be generated in challenge (Not all room ids can be used)|
	...
	|getcurse|int|Bitmask for [curses](../enums/LevelCurse.md) to be forced (Same id's as `cursefilter`).|
	|achievements|string|List of achievement ids that are required to be able to play the challenge.|
	|altpath|bool|Alternative floor for light/dark path. Leave  to allow either path, `true` to force Cathedral/Chest path, or `false` to force Sheol/Dark Room path.|
	|canshoot|bool|Determines if player can shoot<br>Default is `true` (shooting enabled). Cannot force characters who have `canshoot` in their `players.xml` file to shoot, such as Lilith.|
	|redhp|int|Add red hp to the base of the choosen character. 1 = half heart. Negative numbers possible.|
	|maxhp|int|Add red hp containers to the base of the choosen character. 2 = One red heart container. Negative numbers possible.|
	|soulhp|int|Add soul hp containers to the base of the choosen character. 1 = half soul heart.|
	|blackhp|int|Add black hp containers to the base of the choosen character. 1 = half black heart.|
...
???- info ":modding-repentogon: REPENTOGON-exclusive `challenge` tag variables"
	...
	|achievements|string|Same as vanilla, comma-separated list (without spaces) of achievements required to unlock the challenge, you  use achievement names for modded achievements|
...
Many variables available in the `challenges.xml` allow you to change attributes about challenges, however this is limited in what it can provide, and does not support modded content. This wil require Lua code to change.

The first step is to get the ID of your challenge with [Isaac.GetChallengeIdByName](https://wofsauge.github.io/IsaacDocs/rep/Isaac.html#getchallengeidbyname) and checking it against [Isaac.GetChallenge](https://wofsauge.github.io/IsaacDocs/rep/Isaac.html#getchallenge). You can make any desired changes to you challenge through a simple `if` check to see that your challenge is active.
...
You may want to include modded content in your challenge on the player when they first spawn in your challenge. You will need to account for both multiple players, doing it only when starting the challenge as opposed to continuing it, and potentially running this code again when using the Genesis collectible.
