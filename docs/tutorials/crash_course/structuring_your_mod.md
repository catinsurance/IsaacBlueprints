...
???+ note "Identical filepaths across different mods"
	The files that `require` caches is shared across every mod loaded. In a scenario where two or more mods call `require` on the same file with the same filepath, such as `scripts.my_file`, only the first mod's file is successfully loaded, and the second one does not. **If your mod ever uses `require`, give your scripts folder a unique name as to not override identical files from other mods**. A good example is appending the mod name or an abbreviation of it, such as `scripts_mymodname`. `include` does not have this issue due to the lack of cacheing.
